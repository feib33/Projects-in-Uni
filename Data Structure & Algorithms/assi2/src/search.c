...
int game_dijkstra_search(const game_info_t* info,
	...
	while(solution_node == NULL) {
		...
		tree_node_t* one_node = heapq_deque(&pq);
		//Get next color to explore its 4 directions
	        //(use game_next_move_color function in engine.h)
		int next_colour = game_next_move_color(info, &one_node->state);
		for (int dir = DIR_LEFT; dir < 4; ++dir){
			//Check move in that direction is possible 
			//Within the rules of the game (see engine.h)
			if(game_can_move(info, &one_node->state, next_colour, dir)) {
				//Create child node
				tree_node_t* child = node_create(one_node, info, &one_node->state);
				...
				game_make_move(info, &child->state, next_colour, dir);
